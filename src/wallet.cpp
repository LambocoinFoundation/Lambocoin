...
// Lambocoin: optional setting to unlock wallet for block minting only;
...
// Lambocoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Lambocoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // Lambocoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // Lambocoin: not avatar mode
                    {
...
// Lambocoin: create coin stake transaction
...
// Lambocoin: check 'spent' consistency between wallet and txindex
// Lambocoin: fix wallet spent state according to txindex
...
// Lambocoin: disable transaction (only for coinstake)
