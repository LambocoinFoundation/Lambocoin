...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/Lambocoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in Lambocoind / RPC client
            std::string strUsage = _("Lambocoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  Lambocoind [options]                     " + "\n" +
                  "  Lambocoind [options] <command> [params]  " + _("Send command to server or Lambocoind") + "\n" +
                  "  Lambocoind [options] help                " + _("List commands") + "\n" +
                  "  Lambocoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Lambocoin:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("Lambocoin"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("Lambocoin"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: Lambocoin.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: Lambocoind.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // Lambocoin: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // Lambocoin: checkpoint master priv key
    {
    ...
    // Make sure only a single Lambocoin process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Lambocoin is probably already running."), pszDataDir));
    ...
    //
    if (fDaemon)
        fprintf(stdout, "Lambocoin server starting\n");
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("Lambocoin"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Lambocoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Lambocoin to complete") << "\n";
